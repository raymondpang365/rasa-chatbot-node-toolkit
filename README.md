...
│   │   ├── Login.js                # Configuration entry point
...
│   └── Login.js                   # Express server (with webpack dev/hot middlewares)
...
The action(s) will be dispatched through `./src/Login.js` on server-side:
...
```
 |- containers
    |- AsyncRouteComponent
       |- Login.js          // Wrap the route component as async component
       |- RouteComponent.js // The route component
```

The `Login.js` will be:
...
By the way, if you want to use your based style or a vendor CSS framework, just import it through the `./src/containers/App/Login.js` file, for example:

```javascript
import '../../theme/normalize.css'; // Import a vendor stylesheet here
import styles from Home.scss; // Import your based stylesheet here
